...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun distance(other: Circle): Double {
        ...
        return if (distance > radius + other.radius) distance - radius - other.radius else 0.0
    }
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        val x =
            (other.b * sin(angle) - b * sin(other.angle)) / (cos(angle) * sin(other.angle) - cos(
                other.angle
            ) * sin(angle))
        val y = (x * cos(angle) + b) / sin(angle)
        return Point(-y, -x)
    }
...
fun findNearestCirclePair(vararg circles: Circle): Pair<Circle, Circle> {
    require(circles.size >= 2) { "Неверное количество кругов: ${circles.size}" }
    var minDistance = circles[0].distance(circles[1])
    var minPair = Pair(circles[0], circles[1])
...
fun minContainingCircle(vararg points: Point): Circle {
    require(points.isNotEmpty()) { "Множество точек пустое" }
    ...
    return if (mcc.center == thirdPoint) mcc else circleByThreePoints(
        diameter.begin,
        diameter.end,
        thirdPoint
    )
